...
pub struct Default<'a, C = StdClock> {
    map: HashMap<String, Expirable<String>>,
    clock: &'a C,
}
...
impl Default<'_> {
    ...
    pub fn with_clock<C: Clock>(clock: &C) -> Default<C> {
        Default {
            map: HashMap::new(),
            clock,
        }
...
impl<C: Clock> redis::Engine for Default<'_, C> {
    fn call(&mut self, command: redis::Command) -> redis::Result {
        match command {
...
impl<C: Clock> Default<'_, C> {
    fn read<T, R>(&mut self, key: &str, reader: R) -> Option<T>
    where
        R: FnOnce(&Expirable<String>) -> T,
    {
        if let Some(e) = self.map.get(key) {
            if e.is_expired(self.clock.now()) {
                self.map.remove(key);
                return None;
            }
            return Some(reader(e));
        }
        None
    }
    fn entry(
        &mut self,
        key: String,
    ) -> Entry<'_, std::string::String, Expirable<std::string::String>> {
        if let Some(e) = self.map.get(&key) {
            if e.is_expired(self.clock.now()) {
                self.map.remove(&key);
            }
...
mod tests {
    ...
    fn test_set_expiration_seconds() {
        ...
        let mut redis = Default::with_clock(&clock);
